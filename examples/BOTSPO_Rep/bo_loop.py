...
def run_optimization(train_X, train_Y, total_iterations=20):
    ...
    # 1. REMOVED the T_MIN constraint. Now we ONLY constrain max_stress (Index 3).
    constraints = [
        lambda Z: Z[..., 3] - S_MAX  
    ]
    ...
    for iteration in range(start_iter, total_iterations):
        ...
        # 2. REMOVED the historical c1_hist filter. We only filter by stress now.
        c2_hist = train_Y[..., 3] - S_MAX
        is_feasible = (c2_hist <= 0.0)
        
        # Extract only the simulations that physically survived the stress constraint
        feasible_Y = train_Y[is_feasible]
        
        # Build the Pareto front ONLY from feasible objectives
        ...
        # Objectives are at index 0 (-avg_stress) and index 1 (avg_ht)
